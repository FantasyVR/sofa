...
******************************************************************************/
#ifndef SOFA_COMPONENT_COLLISION_NEEDLECONTACT_INL
#define SOFA_COMPONENT_COLLISION_NEEDLECONTACT_INL

#include "NeedleContact.h"
...
NeedleContact<TCollisionModel1,TCollisionModel2,ResponseDataTypes>::NeedleContact()
    : NeedleContact(nullptr, nullptr, nullptr)
{
}


template < class TCollisionModel1, class TCollisionModel2, class ResponseDataTypes  >
NeedleContact<TCollisionModel1,TCollisionModel2,ResponseDataTypes>::NeedleContact(CollisionModel1* model1, CollisionModel2* model2, Intersection* intersectionMethod)
    : model1(model1)
    , model2(model2)
    , intersectionMethod(intersectionMethod)
    , m_unilateralConstraint(nullptr)
    , m_nSlidingConstraint(nullptr)
    , parent(nullptr)
...
template < class TCollisionModel1, class TCollisionModel2, class ResponseDataTypes  >
void NeedleContact<TCollisionModel1,TCollisionModel2,ResponseDataTypes>::cleanup()
{
    if (m_unilateralConstraint || m_nSlidingConstraint)
    {
        m_unilateralConstraint->cleanup();
        m_nSlidingConstraint->cleanup();

        if (parent != nullptr)
        {
            parent->removeObject(m_unilateralConstraint);
            parent->removeObject(m_nSlidingConstraint);
        }

        parent = nullptr;
        m_unilateralConstraint.reset();
        m_nSlidingConstraint.reset();
        mapper1.cleanup();
...
template < class TCollisionModel1, class TCollisionModel2, class ResponseDataTypes  >
void NeedleContact<TCollisionModel1,TCollisionModel2,ResponseDataTypes>::setDetectionOutputs(OutputVector* o)
{
    ...
    {
        msg_error() << "Disabled NeedleContact with " << (outputs.size()) << " collision points.";
        return;
    }
...
template < class TCollisionModel1, class TCollisionModel2, class ResponseDataTypes  >
void NeedleContact<TCollisionModel1,TCollisionModel2,ResponseDataTypes>::activateMappers()
{
    if (!m_unilateralConstraint)
    {
        ...
        // 生成unilateral约束
        m_unilateralConstraint = sofa::core::objectmodel::New<constraintset::UnilateralInteractionConstraint<defaulttype::Vec3Types> >(mmodel1, mmodel2);
        m_unilateralConstraint->setName( getName() );


        setInteractionTags(mmodel1, mmodel2);
        m_unilateralConstraint->setCustomTolerance( tol.getValue() );

        // 生成needle sliding 约束
        m_nSlidingConstraint = sofa::core::objectmodel::New<constraintset::NSlidingConstraint<defaulttype::Vec3Types>>(mmodel1, mmodel2);
        m_nSlidingConstraint->setName(getName());
    }

    auto size = contacts.size();
    m_unilateralConstraint->clear(size);
    m_nSlidingConstraint->clear(size);
    if (selfCollision)
...
template < class TCollisionModel1, class TCollisionModel2, class ResponseDataTypes  >
void NeedleContact<TCollisionModel1,TCollisionModel2,ResponseDataTypes>::createResponse(core::objectmodel::BaseContext* group)
{
    ...
    if (m_unilateralConstraint && m_nSlidingConstraint)
    {
        for (std::vector<sofa::core::collision::DetectionOutput*>::const_iterator it = contacts.begin(); it!=contacts.end(); it++, i++)
        {
            ...
            // 在这里做生成unilateralInteractionConstraint还是nSlidingConstraint的判断，然后生成不同的约束类型。
            // Add contact in unilateral constraint
            if (distance < 0.05)
                m_unilateralConstraint->addContact(mu_, o->normal, distance, index1, index2, index, o->id);
            else
                m_nSlidingConstraint->addContact(mu_, o->normal, distance, index1, index2, index, o->id);
        }
        ...
        if (parent!=nullptr)
        {
            ...
            parent->addObject(m_unilateralConstraint);
            parent->addObject(m_nSlidingConstraint);
        }
    }
}

template < class TCollisionModel1, class TCollisionModel2, class ResponseDataTypes  >
void NeedleContact<TCollisionModel1,TCollisionModel2,ResponseDataTypes>::removeResponse()
{
    if (m_unilateralConstraint && m_nSlidingConstraint)
    {
        mapper1.resize(0);
...
template < class TCollisionModel1, class TCollisionModel2, class ResponseDataTypes  >
void NeedleContact<TCollisionModel1,TCollisionModel2,ResponseDataTypes>::setInteractionTags(MechanicalState1* mstate1, MechanicalState2* mstate2)
{
    ...
    {
        m_unilateralConstraint->addTag(*it);
        m_nSlidingConstraint->addTag(*it);
    }
